...
impl Templates {
    pub fn load() -> Self {
        let file = File::open("resources/template.ron").expect("Failed opening templates file");
    ...
    fn spawn_entity(
        ...
        if let Some(base_damage) = &template.base_damage {
            commands.add_component(entity, Damage(*base_damage));
